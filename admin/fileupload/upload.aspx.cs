        ...
        if (ez.admin.PageBase.chkAdmIP && (ez.admin.PageBase.chkTwIP || ez.admin.PageBase.chkAdmIP_Enable))
        {
            ...
            if (chkuser.isLogin())
            {
                try
                {
                    ...
                    if (ckeditorDir.Substring(ckeditorDir.Length - 1, 1) != "/") { ckeditorDir = ckeditorDir + "/"; }

                    string up_path = Server.MapPath("~/" + ckeditorDir + ValString(Request["dirname"]).Replace("../", ""));
                    ...
                    if (fileSystem.isPhoto(jpeg_image_upload.FileName))
                    {
                        ...
                        if (AutoConvertJPG())
                        {
                            ...
                            savePath = savePath.ToLower().Replace(".tif", ".jpg");
                        }

                        string[] n = savePath.Split('.');

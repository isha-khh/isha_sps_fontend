...
export const demandsApi = {
  ...
  // 取得可供選擇的標籤清單（與企業共用 CompanyTag 分類，含階層 parentId）
  async getTagOptions(): Promise<CompanyTagOption[]> {
    return companiesApi.getCompanyTagOptions();
  },

  // 取得某需求目前綁定的標籤
  async getDemandTags(id: string): Promise<DemandTagsResponse> {
    const response = await apiClient.get(`/api/Demand/${id}/tags`);
    return response.data;
  },

  // 設定某需求的標籤（覆寫綁定，支援多個標籤）
  async setDemandTags(id: string, tagIds: number[]): Promise<DemandTagsResponse> {
    const response = await apiClient.put(`/api/Demand/${id}/tags`, { tagIds });
    return response.data;
  },

  // 取得某需求的通知寄送紀錄
  async getNotifications(id: string): Promise<DemandNotificationRecord[]> {
    const response = await apiClient.get(`/api/Demand/${id}/notifications`);
    return response.data;
  },
};

...
export const memberprofileApi = {
  ...
  // 新增公司成員
  async createMember(request: CreateCompanyMemberRequest): Promise<MemberListItemResponse> {
    const response = await apiClient.post('/api/member/company/members', request);
    return response.data;
  },

  // 更新公司成員
  async updateMember(id: string, request: UpdateCompanyMemberRequest): Promise<MemberListItemResponse> {
    const response = await apiClient.put(`/api/member/company/members/${id}`, request);
    return response.data;
  },

  // 指定/取消指定聯絡人
  async updateDesignatedContact(id: string, isDesignatedContact: boolean): Promise<void> {
    await apiClient.put(`/api/member/company/members/${id}/designated-contact`, { isDesignatedContact });
  },

  // 刪除公司成員
  async deleteMember(id: string): Promise<void> {
    await apiClient.delete(`/api/member/company/members/${id}`);
  },
